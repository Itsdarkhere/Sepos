...
function FooterLink({ name, link, target }) {
  ...
  if (target === "_blank" || link.startsWith("http")) {
    return (
      <a href={link} target={target} rel="noopener noreferrer" className={`${target !== '' && 'underline'} text-neutral-500 hover:text-neutral-400 text-xs font-light`}>
        {name}
      </a>
    );
  }
  return (
    <Link to={link} className={`text-neutral-500 hover:text-neutral-400 text-xs font-light`}>
      {name}
    </Link>
  );
}
